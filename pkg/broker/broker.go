...
func NewBrokerImpl(logger lager.Logger) (bkr *BrokerImpl) {
	...
	return &BrokerImpl{
		...
		Bindings:  map[string]brokerapi.GetBindingSpec{},
		Cflogin: &cfclient.Config{
			ApiAddress: getEnvWithDefault("CF_API_ENDPOINT", "https://api.domain.com"),
			Username: getEnvWithDefault("CF_USER", "user"),
			Password: getEnvWithDefault("CF_PASSWORD", "password"),
  		 },
		Config: Config{
